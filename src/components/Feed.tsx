...
export function Feed({
  ...
  if (listings.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-line bg-surface/40 px-6 py-20 text-center">
        <p className="font-display text-2xl text-fog">No roles in this window.</p>
  ...
  return (
    <div>
      ...
      <div className="py-8 text-center font-mono text-xs text-faint">
        {loading && "loading more…"}
        {!loading && error && (
          <button onClick={loadMore} className="text-amber hover:text-amber-2">
            failed — retry ↻
          </button>
        )}
