...
/**
 * Longest common subsequence over tokens, as a boolean per token of `b`:
 * true when that token also appears in the common subsequence.
 *
...
function commonMask(a: string[], b: string[]): boolean[] {
  const n = a.length;
  const m = b.length;
  if (n === 0 || m === 0) return new Array(m).fill(false);
  if (n * m > 1_000_000) {
